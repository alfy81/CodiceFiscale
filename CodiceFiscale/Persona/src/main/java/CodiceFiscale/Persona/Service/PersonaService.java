...
public class PersonaService {
    ...
    // Istanzio una Mappa per convertire il carattere del mese in un valore numerico
    private static final Map<Character, Integer> meseMap = new HashMap<>();

    static {
        meseMap.put('A', 1);   // Gennaio
        meseMap.put('B', 2);   // Febbraio
        meseMap.put('C', 3);   // Marzo
        meseMap.put('D', 4);   // Aprile
        meseMap.put('E', 5);   // Maggio
        meseMap.put('H', 6);   // Giugno
        meseMap.put('L', 7);   // Luglio
        meseMap.put('M', 8);   // Agosto
        meseMap.put('P', 9);   // Settembre
        meseMap.put('R', 10);  // Ottobre
        meseMap.put('S', 11);  // Novembre
        meseMap.put('T', 12);  // Dicembre
    }

    // Metodo per ottenere le informazioni di una Persona dal Codice Fiscale
    public Persona getPersonaInfo(String codiceFiscale) {
        ...
        LocalDate dataNascita = estraiDataNascita(codiceFiscale);
    ...
    private LocalDate estraiDataNascita(String codiceFiscale) {
        try {
            ...
            int month = meseMap.get(monthChar);
